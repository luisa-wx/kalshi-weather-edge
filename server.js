...
async function fetchMetarData(cityCode) {
    // Try Aviation Weather API first (faster, cleaner JSON)
    const result = await fetchAviationWeatherData(cityCode);
    
    // Optionally also fetch NWS time series for verification
    // const nwsData = await fetchNWSTimeSeries(cityCode);
    // result.nwsData = nwsData;
...
function analyzeHighBrackets(markets, observedHighFloor, observedHighCeiling) {
    return markets.map(market => {
        ...
        // For HIGH markets:
        // - A bracket is DEAD if observedHighFloor > bracket.high (we've already exceeded it)
        // - A bracket is GUARANTEED if observedHighFloor >= bracket.low AND the day is over
        ...
        if (observedHighFloor !== null && high !== null && observedHighFloor > high) {
            status = 'DEAD';
            isDead = true;
        } else if (observedHighFloor !== null && low !== null && observedHighFloor >= low) {
            status = 'IN_RANGE';
        }
        
        // Edge detection for HIGH markets:
        // - YES edge: bracket is dead but YES still has value (confused buyers)
        // - NO edge: could add later for "guaranteed to hit" scenarios
        const yesPrice = market.yes_bid || 0;
        const lastPrice = market.last_price || 0;
        const hasYesEdge = isDead && (yesPrice > 2 || lastPrice > 2);
...
function analyzeLowBrackets(markets, observedLowCeiling, observedLowFloor) {
    return markets.map(market => {
        ...
        // - YES edge: bracket is dead (low already went below it) but YES still has value
        const yesPrice = market.yes_bid || 0;
        const lastPrice = market.last_price || 0;
        const hasYesEdge = isDead && (yesPrice > 2 || lastPrice > 2);
...
app.get('/api/city/:code', async (req, res) => {
    ...
    try {
        // Fetch METAR data
        const { readings, localDate, cityConfig, source } = await fetchMetarData(cityCode);
        ...
        highBrackets.sort((a, b) => (a.low || -999) - (b.low || -999));
        lowBrackets.sort((a, b) => (a.low || -999) - (b.low || -999));
    ...
    for (const [code, config] of Object.entries(CITIES)) {
        try {
            ...
            for (const market of highMarkets) {
                const { high } = parseBracket(market.subtitle || '');
                if (observedHighFloor !== null && high !== null && observedHighFloor > high) {
            ...
            for (const market of lowMarkets) {
                const { low } = parseBracket(market.subtitle || '');
                if (observedLowCeiling !== null && low !== null && observedLowCeiling < low) {
                    if ((market.yes_bid > 2) || (market.last_price > 2)) {
                        lowEdgeCount++;
                    }
